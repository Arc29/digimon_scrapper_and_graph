...
for alpha in range(65,91):
    page = requests.get(digimon_fandom_url+"/wiki/Category:Digimon_species?from="+chr(alpha))
    soup = BeautifulSoup(page.content, 'html.parser')
    results = soup.find(class_='category-page__members-wrapper')
    digimon_row_list.extend(results.find_all('li', class_='category-page__member'))
...
try:
    ...
    for digimon_data in itertools.islice(digimon_row_list, 0, stop):
        tag = digimon_data.find('a',class_='category-page__member-link')
        # print(tag)
        if tag != None:
            if tag.string  in digimon_list or tag.string.startswith('Fan:'):              
                continue
            print('Found new digimon: ',tag.string)
            digi_obj = {'name': '', 'name_ja': '', 'name_ja_romanized': '', 'url': '', 'image': '', 'level': '', 'type': [], 'attribute': [], 'family': [], 'size': [], 'debut': [], 'priorForms': [], 'nextForms': [], 'slideForms': [
            ...
            if tag and tag.has_attr('href'):
                digi_obj['url']=tag['href']
                digimon_page = requests.get(digimon_fandom_url+tag['href'])
                ...
                if info_table != None:
                    ...
                    if info_table.h2 != None:
                        jap_rom_name = info_table.h2.find('span', lang='ja-Latn-hepburn', recursive=True)
                        jap_name = info_table.h2.find('span', lang='ja', recursive=True)
                        if jap_name != None:
                            digi_obj['name_ja'] = jap_name.text
                        if jap_rom_name != None:
                            digi_obj['name_ja_romanized'] = jap_rom_name.text
                    ...
                    for row in rows:
                        ...
                        digi_obj[key] = []
                        eles = row.div
                        if key=='level' or key=='debut':
                            delete_sups(eles)
                            digi_obj[key]=eles.get_text().strip()
                            continue
                        if key=='voiceActors' or key=='partners':
                            # print(eles)
